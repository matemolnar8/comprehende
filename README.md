...
`comprehende serve` and `comprehende export` share one UI and one git payload. Serve resolves refs to commit SHAs when it starts, then computes those payloads from the objects on each request. The prebuilt UI keeps every syntax highlighter as its own file; the browser only fetches the languages the review opens. Export writes the same JSON (and image bytes) next to the UI, then drops highlighter files the review never uses, so any static file server can host the review.
