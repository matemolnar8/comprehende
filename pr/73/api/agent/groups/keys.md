...
Review concern 03 of 03: Bracket keys follow the walk (`keys`)

Part: Group walk

The why:

[#70](source:s1) allows keyboard. `[` and `]` must use the same neighbor as the chrome.
...
Look for:
- Holding `]` does not skip groups. `event.repeat` is ignored.

Depends on:
- 01 Neighbor walk (`walk`)

Hunk refs for this concern:
- src/ui/App.tsx @@ -193,9 +193,11 @@