...
Review concern 01 of 06: Review comparison (`engine`)

Part: Compare reviews

The why:

[#72](source:s1) needs a comparison of interpretation documents, not of the live git diff.
...
Look for:
- Subtle. A group split matches the overlapping group and lists the rest as added. Empty hunk sets never match by overlap.
- When two reviews cite the same source under different ids, equality uses the remapped href. The from-text in the payload stays the original string.

Hunk refs for this concern:
- src/review/compare.test.ts @@ -0,0 +1,295 @@
- src/review/compare.ts @@ -0,0 +1,556 @@