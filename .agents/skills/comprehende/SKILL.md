...
Document `why` names why the work exists, in one or two sentences. It comes only from the sources below; when they are all silent, omit it and let `summary` carry the review. A request naming two unrelated product stories with no unifying source also gets no document `why`; each story keeps its own group `why`s. Never derive document `why` from the diff, from group structure, or from a motive you infer from the patch.

Sources are read, never copied: write your own prose from them, and keep ticket bodies, request descriptions, commit text, and transcript text out of `review.json`.

- Tickets and issues. Copy `id` from the tracker the user named, plus `url`, `title`, and `part` when you have them. Read the ticket (host CLI or MCP, when available) when document `why` needs it. `part` goes on a ticket that belongs to one story, matching that story's group `part`; omit it only when the request or ticket title says the ticket covers the whole review. Commit count proves nothing about ticket scope.
- Request description. When the user named a pull request or merge request, read its description.
- Commit messages. `git log --format='%s%n%n%b' --end-of-options <base>...<head>`. They can clarify group `why`; document `why` needs one of the stronger sources above.
- Transcripts. A coding-agent transcript you already have (this session, or a log the user named) supplies the human's stated reason. Fetching one through a vendor API is out of scope. Only the human's motive counts; the agent's plan, tool trace, and diff recaps stay out.
...
Group `summary` is one sentence saying what the group is, naming how its hunks meet: "the new validator in `core.ts` is invoked by the route in `routes.ts`", not a file-by-file recap or a path list.
